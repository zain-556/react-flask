...
function App() {
  ...
  if (!token) {
    ...
    // FIX: added "register" as a reachable auth page.
    //      Previously there was no way to bootstrap the very first admin from
    //      the login screen — the Create Admin button only appeared after login.
    //      A fresh install with no users had no entry point.
    ...
    <div className="min-h-screen bg-slate-100 font-sans">
      ...
      <header className="bg-slate-800 text-white px-6 py-4 shadow-md">
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col gap-0.5 text-center">
            <h1 className="text-xl font-bold tracking-tight">⚙️ Management Dashboard</h1>
            <p className="text-slate-400 text-xs mt-0.5">Solution for systems.</p>
          </div>
      ...
      {page === "users" && (
        <main className="max-w-4xl mx-auto px-4 py-7">
          ...
          <SpotlightCard user={spotlight} onDismiss={() => setSpotlight(null)} />
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            {showTable && (
              <UsersTable
                ...
                onEdit={(u, mode) => setModal({ user: u, mode })}
              />
            )}
          </div>
        </main>
      ...
      {/* STOCK PAGE */}
      {page === "stock" && <StockManager token={token} />}
